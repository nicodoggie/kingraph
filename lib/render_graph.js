...
function renderPerson(data, house, person, path) {
    ...
    if (
        ...
        if (person.born || person.died) {
            if (person.born && person.died) {
                lifespan = `${'*' + new Date(person.born).getFullYear()} — ${
                    '†' + new Date(person.died).getFullYear()
                }`;
            } else if (person.born) {
                lifespan = `${'*' + new Date(person.born).getFullYear()}`;
            } else {
                lifespan = `${'†' + new Date(person.died).getFullYear()}`;
            }
...
function renderPersonTooltip(person, options) {
    options = {
        locale: 'en-US',
        ...options,
    };
    ...
    txt += person.born
        ? 'Born ' +
          new Date(person.born).toLocaleDateString(locale, {
              day: '2-digit',
              month: '2-digit',
              year: 'numeric',
              timeZone: 'UTC',
    ...
    txt +=
        person.born && person.died
            ? ' -- died ' +
              new Date(person.died).toLocaleDateString(locale, {
                  day: '2-digit',
                  month: '2-digit',
                  year: 'numeric',
                  timeZone: 'UTC',
